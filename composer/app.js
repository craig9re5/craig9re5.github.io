...
function renderPreview() {
  ...
  if (!rawBody.trim()) {
    previewHost.innerHTML = `<p class="preview-empty">实时预览将在此呈现...</p>`;
    return;
  }
...
async function loadPostsIndex() {
  try {
    if (state.engine === ENGINE_LOCAL && state.serviceReady) {
      ...
      // Cloud Engine (GitHub API + Static posts.json + Local Published Cache)
      if (!state.githubToken) {
        state.postsIndex = [];
        renderPostList();
        return true;
      ...
      // 1. Fetch static posts.json generated by Jekyll for rich Chinese titles & metadata
      let staticPosts = [];
      try {
        const staticRes = await fetch(`./posts.json?_t=${Date.now()}`);
        if (staticRes.ok) {
          const list = await staticRes.json();
          if (Array.isArray(list)) staticPosts = list;
        }
      } catch (e) {
        console.warn("Could not load static posts.json:", e);
      }

      // 2. Query GitHub REST API contents/_posts with cache-busting
      const { response, data } = await fetchGhApi(`/repos/${GH_OWNER}/${GH_REPO}/contents/_posts?ref=${GH_BRANCH}&_t=${Date.now()}`);
      if (!response.ok && !staticPosts.length) throw new Error(data.message || `GitHub API 响应 ${response.status}`);
      
      const files = Array.isArray(data) ? data : [];
      state.fileShas.clear();
      files.forEach((f) => state.fileShas.set(f.name, f.sha));

      // Local published cache from current browser session
      const localCache = JSON.parse(localStorage.getItem("post_composer_cloud_cache") || "[]");

      const fileMap = new Map();
      
      // Seed from static posts.json
      staticPosts.forEach((p) => {
        if (p.fileName) fileMap.set(p.fileName, p);
      });

      // Seed from local session cache (newest overrides)
      localCache.forEach((p) => {
        if (p.fileName) fileMap.set(p.fileName, p);
      });

      // Merge files from GitHub API
      ...
      state.postsIndex = Array.from(fileMap.values()).sort((a, b) => {
        const da = a.date || a.fileName.slice(0, 10);
        const db = b.date || b.fileName.slice(0, 10);
  ...
  try {
    if (state.engine === ENGINE_LOCAL && state.serviceReady) {
      ...
      if (!sha && state.mode === "edit") {
        const check = await fetchGhApi(`/repos/${GH_OWNER}/${GH_REPO}/contents/_posts/${encodeURIComponent(fileName)}?ref=${GH_BRANCH}`);
        if (check.response.ok && check.data.sha) {
          sha = check.data.sha;
        }
      }

      const endpoint = `/repos/${GH_OWNER}/${GH_REPO}/contents/_posts/${encodeURIComponent(fileName)}`;
      const { response, data } = await fetchGhApi(endpoint, {
        method: "PUT",
        body: JSON.stringify({
      ...
      // Optimistic instant library cache update
      const newPostItem = {
        fileName,
        slug,
        title: title || slug,
        date: publishInput.value ? publishInput.value.slice(0, 10) : new Date().toISOString().slice(0, 10),
        publishAt: publishInput.value || defaultDateTimeLocal(),
        lang: langInput.value,
        tags: [...state.selectedTags],
        excerpt,
      ...
      const localCache = JSON.parse(localStorage.getItem("post_composer_cloud_cache") || "[]");
      const updatedCache = [newPostItem, ...localCache.filter((p) => p.fileName !== fileName)];
      localStorage.setItem("post_composer_cloud_cache", JSON.stringify(updatedCache.slice(0, 100)));
    }
    ...
    setStatus("🎉 发布成功！已同步至文库并推送到 GitHub。", "success");
